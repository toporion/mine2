...
const ActiveOffers = () => {
  ...
  useEffect(() => {
    const fetchOffers = async () => {
      try {
        const response = await axios.get("http://localhost:8080/api/offer/allActiveOffers"); 
  ...
  const toggleOfferStatus = async (offerId, currentStatus) => {
    try {
      const response = await axios.patch(`http://localhost:8080/api/offer/toggleOffer/${offerId}`);
      if (response.data.success) {
